...
export default function ChatInterface() {
  ...
  return (
    <div dir={direction} className={`flex h-full flex-col ${isIncognitoMode ? 'bg-slate-900' : 'bg-background'}`}>
      {/* Header */}
      <div className={`flex items-center justify-between border-b p-4 ${isIncognitoMode ? 'border-slate-700 bg-slate-800/80' : 'border-border/60 bg-background/80'}`}>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className={`text-lg font-semibold ${isIncognitoMode ? 'text-slate-100' : 'text-foreground'}`}>
                {t("محادثة ذكية", "Smart Chat")}
              </h2>
              <p className={`text-xs ${isIncognitoMode ? 'text-slate-400' : 'text-muted-foreground'}`}>
                {isIncognitoMode
      ...
      {/* Chat Messages */}
      <div className={`flex-1 overflow-y-auto p-4 space-y-4 ${isIncognitoMode ? 'bg-slate-900' : ''}`}>
        {messages.length === 0 && !loading && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            <div className="p-4 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl mb-4">
              <Sparkles className="w-8 h-8 text-white" />
            </div>
            <h3 className={`text-lg font-semibold mb-2 ${isIncognitoMode ? 'text-slate-100' : 'text-foreground'}`}>
              {t("مرحباً بك في SanadGPT", "Welcome to SanadGPT")}
            </h3>
            <p className={`text-sm mb-4 max-w-md ${isIncognitoMode ? 'text-slate-300' : 'text-muted-foreground'}`}>
              {t(
            ...
            <div className="flex gap-2 text-xs">
              <span className={`px-2 py-1 rounded-full ${isIncognitoMode ? 'bg-slate-800 text-slate-300' : 'bg-muted/50 text-muted-foreground'}`}>PDF</span>
              <span className={`px-2 py-1 rounded-full ${isIncognitoMode ? 'bg-slate-800 text-slate-300' : 'bg-muted/50 text-muted-foreground'}`}>Word</span>
              <span className={`px-2 py-1 rounded-full ${isIncognitoMode ? 'bg-slate-800 text-slate-300' : 'bg-muted/50 text-muted-foreground'}`}>Text</span>
            </div>
          </div>
        )}
        ...
        {messages.map((message) => (
          ...
          >
            ...
            >
              ...
              <p className={`text-xs mt-1 px-1 ${isIncognitoMode ? 'text-slate-400' : 'text-muted-foreground'}`}>
        ...
        {loading && (
          <div className="flex gap-3 justify-start">
            ...
            <div className={`rounded-2xl px-4 py-3 ${isIncognitoMode ? 'bg-slate-800 text-slate-100 border border-slate-700' : 'bg-muted/50 text-foreground border border-border'}`}>
              <div className="flex items-center gap-2">
                ...
                ></div>
                <span className={`text-sm ml-2 ${isIncognitoMode ? 'text-slate-300' : 'text-muted-foreground'}`}>
      ...
      {/* Input Area */}
      <div className={`border-t p-4 ${isIncognitoMode ? 'border-slate-700 bg-slate-800/80' : 'border-border/60 bg-background/80'}`}>
        {/* Attachments */}
        {attachments.length > 0 && (
          <div className="mb-3 flex flex-wrap gap-2">
            {attachments.map((file, index) => (
              <div
                key={index}
                className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm ${isIncognitoMode ? 'bg-slate-700 text-slate-200' : 'bg-muted/50'}`}
              >
                <FileText className={`w-4 h-4 ${isIncognitoMode ? 'text-slate-400' : 'text-muted-foreground'}`} />
                <span className="truncate max-w-32">{file.name}</span>
                <span className={`text-xs ${isIncognitoMode ? 'text-slate-400' : 'text-muted-foreground'}`}>
                  ({(file.size / 1024).toFixed(1)} KB)
                </span>
                <Button
                  variant="ghost"
        ...
        <p className={`text-xs mt-2 ${isIncognitoMode ? 'text-slate-400' : 'text-muted-foreground'}`}>
          {t(
